...
Session 位于 `~/.nosis/sessions/<WORKSPACE_KEY>/<SESSION_ID>/`，其中 `<SESSION_ID>.jsonl` 是运行日志，`workspace.json` 保存该 Session 的 Workspace 路径和权限 preset。修改 Workspace 会将整个 Session 目录移动到新的 Workspace 分组。超过回灌上限的 Tool Result 保存为同目录下的 `<TOOL_CALL_ID>.txt`；使用 `nosis --session SESSION_ID` 恢复。子代理的 Journal 写在父 Session 的 `subagents` 目录下，因此不会出现在会话列表中，但仍可查阅。
