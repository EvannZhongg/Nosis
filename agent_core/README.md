...
`shell` 与 `subagent` 接受可选的 `background=true`。后台调用的 Tool Result 立即返回 `{job_id, kind, status}`，真实工作由 Runtime 的 `JobManager` 继续执行，因此不会阻塞当前 Tool Batch。Job 属于创建它的 turn：Agent 可继续模型调用和其他 Tool；如果模型先给出文本而 Job 尚未完成，turn 会保持活动并等待结果。完成、失败或取消结果只在完整 Tool Batch 之外的安全点以 `origin="job_result"` 的 Runtime 消息写入上下文，再触发下一次模型调用。取消 turn 会取消其全部 Job；Shell 会终止进程组，子 Agent 会收到独立取消信号。进程恢复时未完成 Job 标记为 `interrupted`，不会自动重放。
