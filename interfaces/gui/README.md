...
打开历史会话只读取持久化内容，不启动 Bridge。第一次发送消息时，GUI 服务才为该 Session 启动 Runtime。GUI 服务按 `session_id` 保存活动 Runtime；WebSocket 仅负责附着与分离，所以页面刷新或临时断网不会终止正在执行的任务。重新连接后会恢复 running/approval 状态，并按事件序号补发页面还没收到的事件。轮次结束后 Runtime 不退出，而是留给下一轮复用；页面离开、切换模型或工作区、删除会话时，GUI 服务才通过 `DELETE /api/runtimes/{session_id}` 释放它。同一 Session 同时只由一个页面控制；新页面会接管连接，被接管的页面停止自动重连，并可由用户显式接管回来。
