...
Bridge 负责协议路由和交互呈现。共享的应用运行时由 [`agent_runtime`](../../agent_runtime) 提供，Bridge 不再持有装配工厂。
...
Bridge 负责把 `AgentEvent` 和运行状态转换为协议消息，并转发用户输入、授权、提问、取消和状态变更。

一次具体装配由 `ExecutionPlaneManager` 原子持有，包括 Workspace、Provider、已解析 Agent 配置、Workspace Instructions、Agent、MCP、后台 Jobs 和 context window。`SessionRuntimeController` 负责 Session、Workspace、Provider 与权限状态；`TurnRunner` 负责 turn、取消和 steer。Bridge 只把协议消息接到 `RuntimeHost`，定时任务也通过同一 Host 执行。
...
进程入口为 `__main__.py`，协议适配位于 `bridge.py`，运行时服务定义在 `agent_runtime/`。Bridge 可使用内存输入输出进行测试，无需启动 TUI 或 GUI。
