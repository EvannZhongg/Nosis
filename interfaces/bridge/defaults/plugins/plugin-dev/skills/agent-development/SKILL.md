...
description: This skill should be used when the user asks to "create an agent", "add an agent", "write a subagent", "agent frontmatter", "when to use description", "agent examples", "agent tools", "agent colors", "autonomous agent", or needs guidance on agent structure, system prompts, triggering conditions, or agent development best practices for Nosis plugins.
version: 0.1.0
---

# Agent Development for Nosis Plugins
...
Defines when Nosis should trigger this agent. **This is the most critical field** — it is loaded into context whenever the agent is registered, so the harness can decide when to dispatch.
...
Which provider runs the role.

**Values:**
- A provider name from `provider_config.json` (a key of `providers`, e.g. `deepseek`) - pins the role to that provider
- `inherit` - follows the role → subagent → main agent provider chain

**Recommendation:** Use `inherit` unless the role needs a different provider than the one that started it. A value that names no configured provider also inherits, so a typo falls back silently instead of failing.

### color (optional)

Visual identifier for agents, for UIs that render one.

**Options:** `blue`, `cyan`, `green`, `yellow`, `magenta`, `red`

**Note:** Nosis does not consume this field; it has no effect on execution. Include it only for your own organization.
...
**Common tool sets:**
- Read-only analysis: `["read_file", "search_files", "search_files"]`
- Code generation: `["read_file", "write_file", "search_files"]`
- Testing: `["read_file", "shell", "search_files"]`
...
Use this prompt pattern (extracted from Nosis):
...
| model | No | Provider name or inherit | inherit |
| color | No | Color name (ignored by Nosis) | blue |
| tools | No | Array of tool names | ["read_file", "search_files"] |
...
- **`references/agent-creation-system-prompt.md`** - The exact prompt from Nosis
