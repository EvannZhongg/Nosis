...
**You send to Nosis with the system prompt above:**
```
Create an agent configuration based on this request: "I need an agent that reviews pull requests for code quality issues"
```

**Nosis returns JSON (note: prose `whenToUse`, "When to invoke" section in `systemPrompt`):**
```json
{
...
2. Feed to Nosis with this system prompt
