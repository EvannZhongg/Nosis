...
if [ -z "$MODEL" ]; then
  echo "ℹ️  model not set: the role inherits the provider chain"
else
  echo "✅ model: $MODEL"

  case "$MODEL" in
    inherit)
      # Inherits the role → subagent → main agent provider chain
      ;;
    *)
      if ! echo "$MODEL" | grep -Eq '^[a-z][a-z0-9_-]*$'; then
        echo "⚠️  Unknown model: $MODEL (use a provider name from provider_config.json, or inherit)"
        ((warning_count++))
      fi
      ;;
  esac
