...
description: This skill should be used when the user asks about "plugin settings", "store plugin configuration", "user-configurable plugin", ".local.md files", "plugin state files", "read YAML frontmatter", "per-project plugin settings", or wants to make plugin behavior configurable. Documents the .nosis/plugin-name.local.md pattern for storing plugin-specific configuration with YAML frontmatter and markdown content.
version: 0.1.0
---

# Plugin Settings Pattern for Nosis Plugins

## Overview

Plugins can store user-configurable settings and state in `.nosis/plugin-name.local.md` files within the project directory. This pattern uses YAML frontmatter for structured configuration and markdown content for prompts or additional context.

**Key characteristics:**
- File location: `.nosis/plugin-name.local.md` in project root
...
- Prompts to feed back to Nosis
- Documentation or notes
```

### Example: Plugin State File

**.nosis/my-plugin.local.md:**
```markdown
---
enabled: true
strict_mode: false
...
STATE_FILE=".nosis/my-plugin.local.md"
...
Steps:
1. Check if settings exist at `.nosis/my-plugin.local.md`
...
Check for plugin settings at `.nosis/my-plugin.local.md`.
...
```bash
#!/bin/bash
STATE_FILE=".nosis/security-scan.local.md"
...
**.nosis/multi-agent-swarm.local.md:**
```markdown
---
...
2. Create `.nosis/my-plugin.local.md` with YAML frontmatter
3. Set appropriate values based on user input
4. Inform user that settings are saved
5. Remind user to restart Nosis for hooks to recognize changes
```
...
Create `.nosis/my-plugin.local.md` in your project:
...
After creating or editing, restart Nosis for changes to take effect.
```
...
✅ **DO:**
- Use `.nosis/plugin-name.local.md` format
- Match plugin name exactly
- Use `.local.md` suffix for user-local files

❌ **DON'T:**
- Use different directory (not `.nosis/`)
...
**Important:** Settings changes require Nosis restart.
...
After editing `.nosis/my-plugin.local.md`:
1. Save the file
2. Exit Nosis
3. Restart: `nosis`
...
```bash
if [[ ! -f ".nosis/my-plugin.local.md" ]]; then
...
3. Add gitignore entry for `.nosis/*.local.md`
...
7. Remind users that changes require Nosis restart
