...
Detailed analysis of how production plugins use the `.nosis/plugin-name.local.md` pattern.
...
SWARM_STATE_FILE=".nosis/multi-agent-swarm.local.md"
...
```bash
cat > "$WORKTREE_PATH/.nosis/multi-agent-swarm.local.md" <<EOF
---
...
sed "s/^pr_number: .*/pr_number: $PR_NUM/" \
  ".nosis/multi-agent-swarm.local.md" > temp.md
mv temp.md ".nosis/multi-agent-swarm.local.md"
```
...
**Purpose:** Prevent session exit and loop Nosis's output back as input
...
RALPH_STATE_FILE=".nosis/ralph-loop.local.md"
...
# Create state file
cat > ".nosis/ralph-loop.local.md" <<EOF
---
iteration: 1
...
echo "Ralph loop initialized: .nosis/ralph-loop.local.md"
```
...
|---------|-------------------|--------------|
| **File** | `.nosis/multi-agent-swarm.local.md` | `.nosis/ralph-loop.local.md` |
...
```bash
# BAD
FILE="/Users/alice/.nosis/my-plugin.local.md"

# GOOD
FILE=".nosis/my-plugin.local.md"
```
...
The `.nosis/plugin-name.local.md` pattern provides:
