...
4. **Register components**: Make available to Nosis
5. **Initialize**: Start MCP servers, register hooks

**Timing**: Component registration happens during Nosis initialization, not continuously.

### Activation Phase

When components are used:

**Commands**: User types slash command → Nosis looks up → Executes
**Agents**: Task arrives → Nosis evaluates capabilities → Selects agent
**Skills**: Task context matches description → Nosis loads skill
**Hooks**: Event occurs → Nosis calls matching hooks
...
**Note**: Nosis doesn't support nested command discovery automatically. Use custom paths:
...
**Note**: Use build script to combine files, Nosis doesn't support file references.
