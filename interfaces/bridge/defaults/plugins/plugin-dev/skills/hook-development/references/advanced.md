...
if [ -f ".nosis-hooks-config.json" ]; then
  strict_mode=$(jq -r '.strict_mode' .nosis-hooks-config.json)
...
**Example .nosis-hooks-config.json:**
```json
{
  "strict_mode": true,
...
# Append to audit log
echo "$timestamp | $USER | $tool_name | $input" >> ~/.nosis/audit.log
