...
This reference provides common, proven patterns for implementing Nosis hooks. Use these patterns as starting points for typical hook use cases.
  ...
  "Stop": [
    {
      ...
      "hooks": [
        {
          "type": "prompt",
          "prompt": "Review transcript. If code was modified (write_file/edit_file tools used), verify tests were executed. If no tests were run, block with reason 'Tests must be run after code changes'."
        }
      ]
    }
  ]
  ...
  "Stop": [
    {
      ...
      "hooks": [
        {
          "type": "prompt",
          "prompt": "Check if code was modified. If write_file/edit_file tools were used, verify the project was built (npm run build, cargo build, etc). If not built, block and request build."
        }
      ]
    }
  ]
...
FLAG_FILE=".enable-security-scan"
...
**Note:** Must restart Nosis after creating/removing flag files for hooks to recognize changes.
...
```bash
#!/bin/bash
CONFIG_FILE=".nosis/my-plugin.local.json"
...
**Configuration file (.nosis/my-plugin.local.json):**
```json
{
  "strictMode": true,
  "maxFileSize": 500000,
