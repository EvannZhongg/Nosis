...
Practical examples of commands designed for Nosis plugins, demonstrating plugin-specific patterns and features.
...
Note: This uses the subagent tool to launch the plugin's code-reviewer agent for thorough analysis.
```
...
1. **Test with plugin installed:**
   ```bash
   cd /path/to/plugin
   nosis /command-name args
   ```

2. **Verify plugin-relative paths resolve:**
   ```bash
   # List the resources the command expects
   !`ls -la scripts/`
   ```

3. **Test across different working directories:**
   ```bash
   cd /tmp && nosis /command-name
   cd /other/project && nosis /command-name
   ```
...
1. **Using hardcoded absolute paths instead of plugin-relative paths:**
   ```markdown
   # Wrong
   !`node /Users/me/plugins/my-plugin/scripts/analyze.js`
...
2. **Forgetting to allow required tools:**
   ...
   !`bash script.sh`  # Will fail without shell permission
...
4. **Hardcoding plugin paths:**
   ...
   @/home/user/.nosis/plugins/my-plugin/config.json
