...
This reference covers features and patterns specific to commands bundled in Nosis plugins.

## Table of Contents

- [Plugin Command Discovery](#plugin-command-discovery)
- [Plugin-relative paths](#plugin-relative-paths)
...
Nosis automatically discovers commands in plugins using the following locations:
...
2. **Validate file existence:**
   ...
   allowed-tools: shell(test:*), read_file
   ---
...
- Verify every path is relative to the plugin root
...
**Path with spaces:**
- shell commands automatically handle spaces
...
Commands can trigger plugin agents using the subagent tool:
...
Note: This uses the subagent tool to launch the plugin's code-analyzer agent.
```

**Key points:**
- Agent must be defined in plugin's `agents/` directory
- Nosis will automatically use subagent tool to launch agent
...
- Mention skill by name to hint Nosis should invoke it
...
1. shell validation using grep/test
...
1. **Use plugin-relative paths for all plugin-internal paths**
