...
allowed-tools: shell(gh:*), read_file, search_files
---
...
allowed-tools: write_file, shell(git:*)
---
...
Deployment state saved to `.nosis/deployment-state.local.md`:
...
allowed-tools: read_file, shell(npm:*)
---

Reading deployment state from `.nosis/deployment-state.local.md`...
...
allowed-tools: shell(git:*), shell(npm:*), read_file
---
...
```markdown
.nosis/plugin-name-workflow.local.md:
...
Reading workflow state from .nosis/plugin-name-workflow.local.md...

Current stage: @.nosis/plugin-name-workflow.local.md
...
State file: @.nosis/plugin-name-workflow.local.md
...
Creating: .nosis/feature-complete.flag
...
if [ -f .nosis/feature-complete.flag ]; then
...
if [ -f .nosis/deployment.lock ]; then
...
allowed-tools: write_file, shell
---

Deployment complete.

Removing deployment lock...
rm .nosis/deployment.lock
...
!`validate.sh`
echo "checkpoint:validation" >> .nosis/deployment-checkpoints.log

## Checkpoint 2: Build
!`build.sh`
echo "checkpoint:build" >> .nosis/deployment-checkpoints.log

## Checkpoint 3: Deploy
!`deploy.sh`
echo "checkpoint:deploy" >> .nosis/deployment-checkpoints.log
...
Written to .nosis/deployment-state.local.md
...
Reading state: @.nosis/deployment-state.local.md
...
rm .nosis/deployment-state.local.md
