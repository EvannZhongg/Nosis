...
head -n 20 .nosis/commands/my-command.md | grep -A 10 "^---"

# Check for closing frontmatter marker
head -n 20 .nosis/commands/my-command.md | grep -c "^---" # Should be 2

# Verify file has .md extension
ls .nosis/commands/*.md

# Check file is in correct location
test -f .nosis/commands/my-command.md && echo "Found" || echo "Missing"
```
...
if echo "$FRONTMATTER" | grep -q "^model:"; then
  ...
  if ! echo "$MODEL" | grep -Eq '^[a-z][a-z0-9_-]*$'; then
    echo "ERROR: Invalid model '$MODEL' (use inherit, or a provider name from provider_config.json)"
    exit 1
  fi
...
# 5. Check debug logs
tail -f ~/.nosis/debug-logs/latest
...
### Level 6: Shell Execution Testing
...
cat > .nosis/commands/test-bash.md << 'EOF'
---
description: Test bash execution
allowed-tools: shell(echo:*), shell(date:*)
---
...
cat > .nosis/commands/test-forbidden.md << 'EOF'
---
description: Test forbidden command
allowed-tools: shell(echo:*)
---
...
# Command: .nosis/commands/risky-operation.md
...
COMMANDS_CHANGED=$(git diff --cached --name-only | grep "\.nosis/commands/.*\.md")
  ...
  test:
    ...
    steps:
      ...
      - name: Validate command structure
        run: |
          for cmd in .nosis/commands/*.md; do
            echo "Testing: $cmd"
      ...
      - name: Check for TODOs
        run: |
          if grep -r "TODO" .nosis/commands/; then
...
```bash
# Monitor Nosis during command execution
# In terminal 1:
nosis --debug

# In terminal 2:
watch -n 1 'ps aux | grep nosis'
...
```bash
# Check file location
ls -la .nosis/commands/my-command.md

# Check permissions
chmod 644 .nosis/commands/my-command.md

# Check syntax
head -n 20 .nosis/commands/my-command.md
...
```bash
# Verify syntax
grep '\$1' .nosis/commands/my-command.md
grep '\$ARGUMENTS' .nosis/commands/my-command.md

# Test with simple command first
echo "Test: \$1 and \$2" > .nosis/commands/test-args.md
```
...
```bash
# Check allowed-tools
grep "allowed-tools" .nosis/commands/my-command.md

# Verify command syntax
grep '!\`' .nosis/commands/my-command.md
