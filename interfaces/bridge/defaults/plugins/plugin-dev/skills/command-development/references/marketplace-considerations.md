...
if [ ! -f ".nosis/command-initialized" ]; then
...
Checking for user config: .nosis/plugin-name.local.md

if [ -f ".nosis/plugin-name.local.md" ]; then
  # Parse YAML frontmatter for settings
  VERBOSE=$(grep "^verbose:" .nosis/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  COLOR=$(grep "^color:" .nosis/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  MAX_RESULTS=$(grep "^max_results:" .nosis/plugin-name.local.md | cut -d: -f2 | tr -d ' ')

  echo "✓ Using user configuration"
else
  echo "Using default configuration"
  echo "Create .nosis/plugin-name.local.md to customize"
fi
...
Or set in .nosis/plugin-name.local.md:
\`\`\`yaml
---
format: yaml
output: custom.txt
...
if [ -f ".nosis/operation-completed.flag" ]; then
  ℹ️  Operation already completed

  Completed at: $(cat .nosis/operation-completed.flag)

  To re-run:
  1. Remove flag: rm .nosis/operation-completed.flag
  2. Run command again
...
Marking complete...
echo "$(date)" > .nosis/operation-completed.flag
```
