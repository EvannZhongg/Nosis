...
Complete guide to using MCP tools effectively in Nosis plugin commands and agents.

## Overview

Once an MCP server is configured, its tools become available with the prefix `mcp__plugin_<plugin-name>_<server-name>__<tool-name>`. Use these tools in commands and agents just like built-in Nosis tools.
...
- Can use any tool Nosis determines is necessary
...
Nosis automatically structures tool calls based on schema:

```typescript
// Nosis generates this internally
{
...
```markdown
Steps:
1. Make parallel calls (Nosis handles this automatically):
...
2. **Install plugin locally** in `~/.nosis/plugins/`
3. **Verify tools available** with `/mcp`
4. **Test command** that uses tools
5. **Check debug output**: `nosis --debug`
