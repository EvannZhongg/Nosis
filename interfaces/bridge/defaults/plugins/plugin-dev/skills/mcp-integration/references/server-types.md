...
Complete reference for all MCP server types supported in Nosis plugins.
...
1. **Startup**: Nosis spawns process with `command` and `args`
2. **Communication**: JSON-RPC messages via stdin/stdout
3. **Lifecycle**: Process runs for entire Nosis session
4. **Shutdown**: Process terminated when Nosis exits
...
1. **Use plugin-relative paths, or absolute paths when unavoidable**
