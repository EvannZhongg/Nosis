...
A comprehensive toolkit for developing Nosis plugins with expert guidance on hooks, MCP integration, plugin structure, and marketplace publishing.

## Overview

The plugin-dev toolkit provides seven specialized skills to help you build high-quality Nosis plugins:
...
4. **plugin-settings** - Configuration patterns using .nosis/plugin-name.local.md files
...
7. **Testing** - Verify plugin works in Nosis
...
**Trigger phrases:** "create a hook", "add a PreToolUse hook", "validate tool use", "implement prompt-based hooks", "plugin-relative paths", "block dangerous commands"
...
- Environment variable expansion (`${ENV_NAME}`, user vars)
...
**What it covers:**
- .nosis/plugin-name.local.md pattern for configuration
...
- AI-assisted agent generation using Nosis's proven prompt
...
- 3 reference docs: agent-creation-system-prompt (from Nosis), system-prompt-design (~4,000w), triggering-examples (~2,500w)
...
- Based on skill-creator methodology adapted for Nosis plugins
...
This plugin ships with Nosis. On first launch it is copied to `~/.nosis/plugins/plugin-dev`, and Nosis discovers it there on every start.

To keep it installed but inactive, set `"enabled": false` in that `plugin.json`. To remove it, delete the directory.
...
This keeps Nosis's context focused while providing deep knowledge when needed.
...
- Based on official Nosis documentation
...
✅ **Portability**
- Use plugin-relative paths everywhere
- Relative paths only
...
The source of this plugin is `interfaces/bridge/defaults/plugins/plugin-dev/` in the Nosis repository. Change it there:

1. Edit the source copy in the repository
2. Delete `~/.nosis/plugins/plugin-dev` so the next launch installs the current files
3. Restart Nosis
