...
Guide the user through creating a complete, high-quality Nosis plugin from initial concept to tested implementation. Follow a systematic approach: understand requirements, design components, clarify details, implement following best practices, validate, and test.

## Core Principles

- **Ask clarifying questions**: Identify all ambiguities about plugin purpose, triggering, scope, and components. Ask specific, concrete questions rather than making assumptions. Wait for user answers before proceeding with implementation.
- **Load relevant skills**: Use the read_skill tool to load plugin-dev skills when needed (plugin-structure, hook-development, agent-development, etc.)
...
- **Use update_plan**: Track all progress throughout all phases
...
**MUST load plugin-structure skill** using read_skill tool before this phase.
...
- What tools does it need (read_file, write_file, shell, etc.)?
...
4. Create plugin.json manifest using write_file tool:
   ...
   ```
   Only these seven fields exist: `name`, `version`, `description`, `enabled`, `dependencies`, `capabilities`, `components`. Nosis skips the whole plugin if the manifest carries any other field.
5. Create README.md template
6. Create .gitignore if needed (for .nosis/\*.local.md, etc.)
...
1. Load skill-development skill using read_skill tool
2. For each skill:
   ...
   - For user-invoked skills (slash commands): include `description`, `argument-hint`, and `allowed-tools` frontmatter; write instructions FOR Nosis (not TO user)
...
1. Load command-development skill using read_skill tool
2. For each command:
   - Write command markdown with frontmatter
   - Include clear description and argument-hint
   - Specify allowed-tools (minimal necessary)
   - Write instructions FOR Nosis (not TO user)
   - Provide usage examples and tips
   - Reference relevant skills if applicable

### For Agents:

1. Load agent-development skill using read_skill tool
2. For each agent, use agent-creator agent:
   ...
   - Add appropriate model and tools
   - Validate with validate-agent.sh script

### For Hooks:

1. Load hook-development skill using read_skill tool
2. For each hook:
...
1. Load mcp-integration skill using read_skill tool
2. Create .mcp.json configuration with:
   - Server type (stdio for local, SSE for hosted)
   - Command and args (plugin-relative paths)
...
1. Load plugin-settings skill using read_skill tool
2. Create settings template in README
3. Create example .nosis/plugin-name.local.md file (as documentation)
4. Implement settings reading in hooks/commands as needed
5. Add to .gitignore: `.nosis/*.local.md`
...
5. **Test hook configuration** (if plugin has hooks):
   ...
   - Verify plugin-relative path usage
...
**Goal**: Test that plugin works correctly in Nosis
...
1. **Installation instructions**:
   - Show user how to test locally:
     ```bash
     nosis --plugin-dir /path/to/plugin-name
     ```
   - Or copy to `~/.nosis/plugins/` for local testing
...
3. **Testing recommendations**:
   ...
   - For hooks: Use `nosis --debug` to see hook execution
...
- **Load skills with read_skill tool** when working on specific component types
...
- **Apply best practices**:
  ...
  - Skill instructions written FOR Nosis (not TO user)
  - Strong trigger phrases
  - Plugin-relative paths for portability
